// Game.cpp Starts Here

/* Contains ::   Class game
                 Functions assigned for the Flow
...
//////////////* Default Constructor *////
// Registering Class For Game Flow
...
bool Game::dealDealer()         /// Function For Dealer Sum Win
{
...
char Game::compareSum()     // Function for Comparing Sum
{
    if(player.getSum()>dealer.getSum())         // Condition For High Player Win
    {
        printTop();
        std::cout<<lightYellow<<Print::you_win()<<def<<"\n    (Dealer has "<<dealer.getSum()<<")\n";
        return 'p';
    }
    else if(dealer.getSum()>player.getSum())      // Condition for Player Lose
    {
        printTop();
        ...
        printTop();
        std::cout<<lightMagenta<<Print::draw()<<def;    // Condition for draw
        return 'n';
    }
}

bool Game::checkWins()          // Function For Cash Increment/Decrement
{
    switch(checkEnd())
    {
        case 'f': return false;
...
void Game::printTop(){
    clearscr();
    std::cout<<yellow<<Print::title_blackjack()<<def<<"\n";
    std::cout<<lightRed<<"\t\tCards: "<<deck.DeckStrength()<<lightGreen<<" \tCash: "<<player.getCash()<<lightMagenta
